...
from .validate_matrix import load_transition_role_matrix
from .validate_registry import load_transition_role_registry
...
def _build_item(role: dict[str, Any], matrix_rows: list[dict[str, Any]], results_root: Path) -> CoverageItem:
    ...
    workload_entries = list(row.get("workloads", [])) if isinstance(row, dict) else []
...
def build_role_coverage_report(
    ...
    registry = load_transition_role_registry(roles_file)
    matrix = load_transition_role_matrix(matrix_file)
    items = [_build_item(role, matrix.get("rows", []), results_dir) for role in registry.get("roles", []) if isinstance(role, dict)]
