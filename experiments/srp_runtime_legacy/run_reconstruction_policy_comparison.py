...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the fixed SRP reconstruction policy comparison.")
    parser.add_argument(
        "--suite",
        action="append",
        choices=["all", "unrestricted", "constrained", "minimal"],
        default=[],
        help="Reconstruction policy suite to run. Repeatable. Defaults to all suites.",
    )
    parser.add_argument("--cycles", type=int, default=1, help="Cycles per task.")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=PROJECT_ROOT / "srp_experiment" / "tmp" / "reconstruction_policy",
...
def main() -> int:
    ...
    records = run_reconstruction_policy_comparison(suites, cycles=args.cycles)
    summary = summarize_reconstruction_policy_comparison(records)

    if not args.no_write:
        outputs = write_reconstruction_policy_outputs(records, args.output_dir)
