...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "inputs",
        nargs="*",
        default=[PROJECT_ROOT / "experiments" / "results" / "analysis" / "graph_recovery_ablation" / "graph_recovery_ablation_records.jsonl"],
        help="Input JSONL/JSON files or directories containing graph recovery records.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=PROJECT_ROOT / "experiments" / "results" / "analysis" / "graph_information_gap_analysis",
