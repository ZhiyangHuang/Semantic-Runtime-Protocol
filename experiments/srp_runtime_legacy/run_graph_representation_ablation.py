...
from .graph_representation_ablation_harness import (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the SRP graph representation ablation harness.")
    parser.add_argument(
        "--suite",
        action="append",
        choices=["all", *available_suite_names()],
        default=[],
        help="Graph representation suite to run. Repeatable. Defaults to all suites.",
    )
    parser.add_argument("--cycles", type=int, default=1, help="Cycles per task.")
    parser.add_argument("--seeds", type=int, default=5, help="Seed samples per suite.")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=PROJECT_ROOT / "srp_experiment" / "tmp" / "graph_representation_ablation",
...
def main() -> int:
    ...
    records = run_graph_representation_ablation(suites, cycles=args.cycles, seeds=args.seeds)
    summary = summarize_graph_representation_ablation(records)

    if not args.no_write:
        outputs = write_graph_representation_ablation_outputs(records, args.output_dir)
