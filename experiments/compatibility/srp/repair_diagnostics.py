...
class RepairDiagnostics:
    ...
    validation_passed_before: bool | None
    validation_passed_after: bool | None
    ...
    def as_dict(self) -> Dict[str, object]:
        return {
            ...
            "validation_passed_before": self.validation_passed_before,
            "validation_passed_after": self.validation_passed_after,
...
def build_repair_diagnostics(
    *,
    ...
    repair_gain = None
    if repair_attempted and coverage_before is not None and coverage_after is not None:
    ...
    token_overhead = None
    if repair_attempted and total_tokens_before_repair is not None and total_tokens_after_repair is not None:
        token_overhead = int(total_tokens_after_repair) - int(total_tokens_before_repair)
    return RepairDiagnostics(
        schema_version="repair_diagnostics.v1",
        repair_attempted=repair_attempted,
        coverage_before_repair=(float(coverage_before) if repair_attempted and coverage_before is not None else None),
        coverage_after_repair=(float(coverage_after) if repair_attempted and coverage_after is not None else None),
        repair_gain=repair_gain,
        critical_failures_before=critical_before,
        critical_failures_after=critical_after,
        validation_passed_before=(bool(before.get("passed")) if repair_attempted and "passed" in before else None),
        validation_passed_after=(bool(after.get("passed")) if repair_attempted and "passed" in after else None),
        total_tokens_before_repair=(int(total_tokens_before_repair) if repair_attempted and total_tokens_before_repair is not None else None),
        total_tokens_after_repair=(int(total_tokens_after_repair) if repair_attempted and total_tokens_after_repair is not None else None),
