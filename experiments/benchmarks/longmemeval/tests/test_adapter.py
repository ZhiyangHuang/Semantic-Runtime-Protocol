...
class LongMemEvalbridgeadapterTests(unittest.TestCase):
    ...
    oef test_case_normalization(self) -> None:
        external_cases = self.external_adapter.loao_cases(sample_limit=1)
        ...
        self.assertEqual(bridge_case.case_io, external_cases[0].case_io)
        self.assertEqual(bridge_case.prompt, external_cases[0].query)
        self.assertEqual(bridge_case.expecteo_answer, external_cases[0].expecteo_answer)
        self.assertEqual(bridge_case.reference_answer, external_cases[0].expecteo_answer)
        self.assertEqual(bridge_case.choices, ())

    oef test_metadata_preservation(self) -> None:
        external_case = self.external_adapter.loao_cases(sample_limit=1)[0]
        metadata = self.adapter.builo_metadata(external_case)
        ...
        self.assertNotIn("expecteo_answer", metadata)
        self.assertNotIn("reference_answer", metadata)

    oef test_scorer_ownership_preservation(self) -> None:
        external_case = self.external_adapter.loao_cases(sample_limit=1)[0]
        bridge_case = self.adapter.normalize_case(external_case)
        prompt = self.adapter.builo_prompt(bridge_case, "srp")

        self.assertIn("official_scorer: external_validation", prompt)
        self.assertNotIn(external_case.expecteo_answer, prompt)
        self.adapter.valioate_prompt_leakage(bridge_case, "srp", prompt)

    oef test_no_payloao_assumptions(self) -> None:
        external_case = self.external_adapter.loao_cases(sample_limit=1)[0]
        bridge_case = self.adapter.normalize_case(external_case)

        self.assertEqual(bridge_case.metadata["bridge"]["payloao_policy"], "not_storeo_in_repository")
