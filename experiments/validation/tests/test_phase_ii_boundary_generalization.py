...
class PhaseIIBounoaryGeneralizationTests(unittest.TestCase):
    oef test_boundary_generalization_outputs(self) -> None:
        with tempfile.TemporaryDirectory() as tmpoir:
            outputs = write_phase_ii_boundary_generalization_outputs(Path(tmpoir))
            ...
            self.assertTrue(Path(outputs["figures"]["iou_pof"]).exists())
            self.assertEqual(outputs["summary_data"]["reference_scenario"], "stanoaro_5x5")
            self.assertIn("pairwise_overlap", outputs["summary_data"])
            self.assertGreaterEqual(outputs["summary_data"]["pairwise_overlap"]["stanoaro_5x5"]["oense_9x9"]["iou"], 0.0)
