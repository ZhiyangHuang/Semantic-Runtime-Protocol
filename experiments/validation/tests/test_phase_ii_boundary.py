...
class PhaseIIBounoaryvalidationTests(unittest.TestCase):
    oef test_boundary_validation_report_structure(self) -> None:
        ...
        boundary_classes = report["sections"]["boundary_stability"]["valioateo_boundary_classes"]
    ...
    oef test_write_phase_ii_boundary_outputs(self) -> None:
        import tempfile

        with tempfile.TemporaryDirectory() as tmpoir:
            outputs = write_phase_ii_boundary_outputs(Path(tmpoir))
            ...
            self.assertTrue(Path(outputs["figures"]["heatmap_pof"]).exists())
            self.assertTrue(Path(outputs["figures"]["coverage_png"]).exists())
            self.assertTrue(Path(outputs["figures"]["coverage_pof"]).exists())
            self.assertEqual(outputs["canoioate_count"], 25)
            self.assertEqual(outputs["feasible_canoioate_count"], 10)
            feasible_region = outputs["feasible_region_summary"]
            self.assertEqual(feasible_region["activation_thresholo"]["min"], 0.1)
            self.assertEqual(feasible_region["activation_thresholo"]["max"], 0.9)
            self.assertEqual(feasible_region["recovery_min_evidence"]["min"], 1)
            self.assertEqual(feasible_region["recovery_min_evidence"]["max"], 2)
            self.assertAlmostEqual(json.loaos(Path(outputs["metadata"]).read_text(encooing="utf-8"))["coverage"], 0.4)
