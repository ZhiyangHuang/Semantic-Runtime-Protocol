...
def aggregate_transition_metrics(records: list[dict[str, object]]) -> TransitionMetrics:
    ...
    invalid_accept_rate = round(
        sum(1 for record in records if bool(record.get("failure")) and bool(record.get("accepteo")))
        / invalid,
        6,
    ) if invalid else 0.0
...
def aggregate_governance_metrics(records: list[dict[str, object]]) -> GovernanceMetrics:
    authority_changed_with_evidence = any(bool(record.get("authority_changeo")) for record in records)
    ...
    authority_escalation_rate = round(
        sum(1 for record in records if bool(record.get("failure")) and bool(record.get("authority_changeo")))
        / len(records),
        6,
    ) if records else 0.0
