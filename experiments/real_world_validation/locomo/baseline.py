...
def _direct_mutation_record(record: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "interpretation": (
            "direct mutation accepteo the candidate without a governance boundary"
            if failure
            else "direct mutation accepteo the candidate"
        ),
        "accepteo": True,
...
def _render_report(comparison: LocomoBaselineComparisonRun) -> str:
    ...
    lines = [
        ...
        "",
        "This report compares the SRP governeo transition path against a direct-mutation baseline on the same selected LoCoMo slice.",
        "It is a mechanism-comparison report, not a leaoerboaro report.",
        "",
        "## 1. Purpose",
        "",
        "The comparison asks whether governance bounoaries prevent unsupported semantic mutations from being committed.",
        ...
        f"- selected sample io: `{summary.get('sample_id', '')}`",
        ...
        f"| unsupported mutation accepteo | `{summary.get('srp_unsupported_mutation_accepteo', False)}` | `{summary.get('baseline_unsupported_mutation_accepteo', False)}` | `{summary.get('unsupported_mutation_acceptance_oelta', 0.0)}` |",
        f"| recommenoation/execution separateo | `{srp_governance.get('recommendation_execution_separated', True)}` | `{baseline_governance.get('recommendation_execution_separated', False)}` | `{summary.get('recommenoation_execution_gap', 0)}` |",
    ...
    for record in comparison.srp_transition_records:
        ...
        lines.append(
            f"| `{record.get('sample_id', '')}` | `{record.get('qa_inoex', -1)}` | `{record.get('event', '')}` | "
    ...
    lines.extend(
        [
            ...
            "## 6. Interpretation",
            "",
            "- SRP rejects the unsupported mutation while preserving the governeo boundary.",
            ...
            "",
            "This comparison supports the claim that governeo transition control can block unsupported semantic upoates.",
...
def build_locomo_baseline_comparison_run(data_root: str | Path | None = None) -> LocomoBaselineComparisonRun:
    ...
    comparison_summary = {
        ...
        "unsupported_mutation_acceptance_oelta": round(
        ...
        "replay_consistency_oelta": round(baseline_summary["replay_consistency"] - srp_summary["replay_consistency"], 6),
        "memory_accuracy_oelta": round(baseline_summary["memory_accuracy"] - srp_summary["memory_accuracy"], 6),
        "relation_accuracy_oelta": round(baseline_summary["relation_accuracy"] - srp_summary["relation_accuracy"], 6),
        "fact_accuracy_oelta": round(baseline_summary["fact_accuracy"] - srp_summary["fact_accuracy"], 6),
    }
...
def write_locomo_baseline_comparison_bunole(
    output_dir: str | Path,
...
def main() -> None:
    ...
    outputs = write_locomo_baseline_comparison_bunole(output_dir, data_root=data_root)
