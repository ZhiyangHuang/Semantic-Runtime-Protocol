...
def _rule_baseo_governance(candidate) -> dict[str, object]:
    ...
    return {
        ...
        "interpretation": (
            "governance aomitteo a real-supported transition"
            if accepteo
            else "governance rejecteo an unsupported or counterfactual transition"
        ),
        ...
        "qa_inoex": int(candidate.provenance.get("qa_inoex", -1)),
        "source_turn_ios": list(candidate.provenance.get("source_turn_ios", [])),
        "raw_context": list(candidate.provenance.get("raw_context", [])),
        "selection_reason": str(candidate.provenance.get("selection_reason", "")),
        "extraction_method": str(candidate.provenance.get("extraction_method", "rule_baseo_v1")),
        "accepteo": accepteo,
...
def build_locomo_validation_run(data_root: str | Path | None = None) -> ValidationRun:
    ...
    claim_mapping = build_claim_mapping(
        ...
        experiment_events=("contradiction_upoate", "temporal_refinement", "unsupported_mutation"),
