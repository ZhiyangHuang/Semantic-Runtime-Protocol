...
def _event_type_for_case(case: BenchmarkCase, fallback_index: int) -> str:
    text = _case_text(case)
    if any(token in text for token in ("preference", "workspace", "standing desk", "quiet room")):
        return "preference_revision"
    if any(token in text for token in ("contradiction", "no longer", "replaced", "available", "conflict")):
        return "contradiction_resolution"
    return "preference_revision" if fallback_index == 0 else "contradiction_resolution"
...
def _build_supported_event(case: BenchmarkCase, event_type: str, source_mode: str) -> SelectedLongMemEvalEvent:
...
def _build_counterfactual_event(case: BenchmarkCase, source_mode: str) -> SelectedLongMemEvalEvent:
    evidence_unit_ids, evidence_relation_ids = collect_case_evidence(case)
...
def select_longmemeval_events(
    ...
    source_mode = str(manifest.get("source_mode", "fixture_fallback"))
    ...
    for case in cases:
        ...
        selected_events.append(_build_supported_event(case, event_type, source_mode))
        if len(selected_events) >= 2:
            break

    if selected_cases:
        selected_events.append(_build_counterfactual_event(selected_cases[0], source_mode))

    if sample_limit and sample_limit > 0:
        selected_events = selected_events[:sample_limit]

    manifest = dict(manifest)
    manifest["selected_samples"] = len({event.benchmark_case_id for event in selected_events if event.probe_mode != "counterfactual"})
