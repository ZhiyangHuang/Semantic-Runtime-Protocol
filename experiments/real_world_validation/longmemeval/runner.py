...
def _rule_based_governance(candidate) -> dict[str, object]:
    ...
    evidence_improvement = 0.12 + 0.08 * evidence_count if accepted else 0.04
    return {
        ...
        "interpretation": (
            "governance admitted a supported LongMemEval transition"
            if accepted
            else "governance rejected an unsupported or counterfactual LongMemEval transition"
        ),
        ...
        "source_mode": str(candidate.provenance.get("source_mode", "")),
        ...
        "memory_accuracy": 1.0 if accepted else 0.0,
        "relation_accuracy": 1.0 if accepted else 0.0,
        "fact_accuracy": 1.0 if accepted else 0.0,
        "coverage": 1.0,
    }
...
def build_longmemeval_validation_run(data_root: str | Path | None = None) -> ValidationRun:
    ...
    decision = make_decision(
        ...
        claim_scope="evaluated LongMemEval subset",
    )

    claim_mapping = build_claim_mapping(
        claim_id="authority_independence",
        paper_section="3.5",
        observable_behavior="stronger evidence may refine verification without changing authority or collapsing rejection boundaries",
    ...
    dataset_manifest = build_dataset_manifest(
        ...
        selected_samples=int(manifest.get("selected_samples", len({event.benchmark_case_id for event in selected_events if event.probe_mode != "counterfactual"}))),
