...
from ..srp.semantic_parser import canonicalize_semantic_value, stable_semantic_object_id
...
def _provenance_signature(item: Dict[str, Any]) -> str:
    provenance = item.get("provenance") if isinstance(item.get("provenance"), dict) else {}
    if not provenance:
        return ""
    keys = ["source_document", "turn", "sentence", "token_span", "extraction_method", "reasoning_path", "compression_round", "recovery_mode", "validation_outcome", "evidence_pointer"]
    parts = [f"{key}={_safe_str(provenance.get(key))}" for key in keys if provenance.get(key) not in (None, "", [], {})]
    return "|".join(parts)
