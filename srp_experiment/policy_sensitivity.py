...
def summarize_policy_sensitivity(records: Sequence[Dict[str, Any]], axes: Sequence[SensitivityAxis] | None = None) -> Dict[str, Any]:
    selected_axes = list(axes) if axes is not None else build_sensitivity_axes()
...
def render_policy_sensitivity_markdown(summary: Dict[str, Any]) -> str:
    lines = ["# Policy Sensitivity Analysis", ""]
...
def write_policy_sensitivity_outputs(records: Sequence[Dict[str, Any]], output_dir: str | Path) -> Dict[str, Path]:
    output_path = Path(output_dir)
    output_path.mkdir(parents=True, exist_ok=True)
    jsonl_path = output_path / "policy_sensitivity_records.jsonl"
    csv_path = output_path / "policy_sensitivity_records.csv"
    markdown_path = output_path / "policy_sensitivity_audit.md"
    summary_path = output_path / "policy_sensitivity_summary.md"
    ...
    summary_path.write_text(render_policy_sensitivity_markdown(summary), encoding="utf-8")
